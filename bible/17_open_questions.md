...
- ~~**Trial Badge naming convention**~~ — **RESOLVED in v0.9.8**: Every Trial Badge will have a name. Trial 5's badge is named **Firmament** (locked). The other 8 Trial Badge names are `[OPEN]` and to be locked per-Trial as the Trial Baron entries are completed.
