...
static bool8 LoadBattlerSpriteGfx(u32 battler)
{
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            DecompressTrainerBackPic(TRAINER_BACK_PIC_OSRID, battler);
...
void CreateBattlerSprite(u32 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(TRAINER_BACK_PIC_OSRID, GetBattlerPosition(B_POSITION_PLAYER_LEFT));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, 0x50,
                                                (8 - gTrainerBacksprites[TRAINER_BACK_PIC_OSRID].coordinates.size) * 4 + 80,
