...
bible/INDEX.md — fetch this FIRST. It's a ~2K-token index that lists 19 per-section files with summaries and token estimates. Read the index, decide which 1-3 sections your current task actually requires, and fetch only those.
...
From the bible Section 9 "The Jousteel Line" (fetch bible/13_cosmology_mechanics.md for full content):
...
The bible Section 14 (fetch bible/18_engineering_tasks.md) has an engineering entry for "Jousteel line full Pokemon data + sprite work" tagged [MEDIUM]. The sprite work is the Phase 2a deliverable.
...
bible/INDEX.md — index of split bible sections; from here decide what bible content to fetch
For Phase 2 specifically: bible/05_starter_lines.md (starter palette work context), bible/07_characters_viziers.md (Cadmus Umbra), and bible/13_cosmology_mechanics.md (Jousteel line cosmology + custom species patterns)
